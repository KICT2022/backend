...
public class DrugRecommendationService {
    ...
    public void init() {
        this.webClient = WebClient.builder()
                ...
                .build();
        log.info("DrugRecommendationService initialized with URL: {}", apiUrl);
    }

    public String getChatCompletion(ChatCompletionRequest request) {
        if (request == null || request.getMessages() == null || request.getMessages().isEmpty()) {
            log.error("ChatCompletionRequest 객체 또는 messages 리스트가 null이거나 비어 있습니다.");
            return "요청 데이터가 올바르지 않습니다. 다시 시도해 주세요.";
        }

        log.info("OpenAI API 요청 시작, 모델: {}, 메시지 수: {}", request.getModel(), request.getMessages().size());
        ...
        try {
            ...
            if (response != null && response.getChoices() != null && !response.getChoices().isEmpty()) {
                ...
                log.warn("OpenAI API 응답에 choices가 없거나 비어있음");
